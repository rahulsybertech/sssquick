...
class FirmAdapter(
    ...
    purchasePartyData: List<AccountName>,
)
    : ArrayAdapter<AccountName>(mContext, mLayoutResourceId, purchasePartyData) {

    private val purchaseParty: MutableList<AccountName> = ArrayList(purchasePartyData)
    private var allPurchaseParty: List<AccountName> = ArrayList(purchasePartyData)

    // Method to update the list of items dynamically
    fun updateData(newData: List<AccountName>) {
        purchaseParty.clear()
    ...
    override fun getItem(position: Int): AccountName = purchaseParty[position]
    ...
    override fun getFilter(): Filter {
        return object : Filter() {
            override fun convertResultToString(resultValue: Any): String? {
                return if(!type){
                    (resultValue as AccountName).name
                }else{
                    (resultValue as AccountName).name
                }
            }
            ...
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                if (constraint != null) {
                    val purchasePartySuggestion: MutableList<AccountName> = ArrayList()
