...
class StationAdapter(
    ...
    purchasePartyData: List<StationItem>,
)
    : ArrayAdapter<StationItem>(mContext, mLayoutResourceId, purchasePartyData) {

    private val purchaseParty: MutableList<StationItem> = ArrayList(purchasePartyData)
    private var allPurchaseParty: List<StationItem> = ArrayList(purchasePartyData)

    // Method to update the list of items dynamically
    fun updateData(newData: List<StationItem>) {
        purchaseParty.clear()
    ...
    override fun getItem(position: Int): StationItem = purchaseParty[position]
    ...
    override fun getFilter(): Filter {
        return object : Filter() {
            override fun convertResultToString(resultValue: Any): String? {
                return if(!type){
                    (resultValue as StationItem).name
                }else{
                    (resultValue as StationItem).name
                }
            }
            ...
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                if (constraint != null) {
                    val purchasePartySuggestion: MutableList<StationItem> = ArrayList()
