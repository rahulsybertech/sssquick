...
class DashBoardRepository @Inject constructor(private val apiService: ApiService) {
    ...
    suspend fun callGetDashBoardDetails(marketerId: String) : ResultWrapper<ApiResponse<*>, ApiResponse<DashBoardDataResponse>> {
        return safeApiCall(Dispatchers.IO, ApiRequestCode.DASHBOARD_DATA.ordinal) {
            apiService.getDashBoardData(marketerId = marketerId)
        }

    }
    suspend fun callGetDashBoardSaleCountDetails(marketerId: String, fromDate: String, toDate: String) : ResultWrapper<ApiResponse<*>, ApiResponse<DashBoardDataResponse>> {
        return safeApiCall(Dispatchers.IO, ApiRequestCode.DASHBOARD_DATA.ordinal) {
